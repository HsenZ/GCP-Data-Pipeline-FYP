...
ODS_DATASET = 'ods_ds'
STAGING_TABLE = 'stg_day_earthquake'
...
class TransformToODS(beam.DoFn):
    ...
    def process(self, row):
        try:
            ...
            vl_n_depth = depth + 0.5 * depth_error if depth_error else depth
            vl_n_mag = mag + 0.5 * mag_error if mag_error else mag
            ...
            print(f"[ERROR] Skipping row of type={row.get('type')}, place={row.get('place')}, error={e}, row={row}")
...
def run():
    ...
    gco.region = REGION
    gco.job_name = f'ods-earthquake-transform-{timestamp.lower()}'
