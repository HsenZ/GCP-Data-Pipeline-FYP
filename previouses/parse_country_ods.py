...
class ParsePlaceDoFn(beam.DoFn):
    def process(self, row, states_dict):
        ...
        # ✅ Set current timestamp to _DT_updated_date (in EEST)
        EEST = timezone(timedelta(hours=3))
        now_str = datetime.now(EEST).strftime("%Y-%m-%d %H:%M:%S")
        updated_row["_DT_updated_date"] = now_str
        ...
        if "," not in place:
            ...
            if state_match:
                ...
                rr = match.group(1).strip() if match else None
...
def run():
    ...
    gco.region = REGION
    gco.job_name = 'parse-country-ods-pipeline'
