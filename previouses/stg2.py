...
class ParseCSVLine(beam.DoFn):
    def process(self, line):
        ...
        row['original_csv_line'] = line  # keep for rejection
        yield row
...
class FilterTransform(beam.DoFn):
    def process(self, row):
        try:
            ...
            row.pop('original_csv_line', None)  # remove before writing to BigQuery
...
def run():
    ...
    with beam.Pipeline(options=options) as p:
        ...
        # ✅ Write accepted rows to BigQuery (predefined table, truncate old data)
        ...
        rejected | 'Write rejected to GCS' >> beam.io.WriteToText(
            ...
            shard_name_template=''  # write one file
        )
