...
def run(argv=None):
    ...
    with beam.Pipeline(options=options) as p:
        ...
        def enrich_fact(row, net_map, reg_map, type_map, mag_map, depth_map):
            ...
            insertion_date = None
            if '_DT_insertion_date' in row and row['_DT_insertion_date']:
                insertion_date = parser.parse(row['_DT_insertion_date']).replace(tzinfo=None) if isinstance(row['_DT_insertion_date'], str) else row['_DT_insertion_date'].replace(tzinfo=None)
