...
class ParsePlaceDoFn(beam.DoFn):
    ...
    def process(self, row, states_dict):
        ...
        # set updated timestamp (EEST)
        EEST = timezone(timedelta(hours=3))
        updated_row["_DT_updated_date"] = datetime.now(EEST).strftime("%Y-%m-%d %H:%M:%S")
    ...
    with beam.Pipeline(options=options) as p:
        ...
        (
            ...
            | 'Write OUTPUT' >> WriteToBigQuery(
                ...
                write_disposition=BigQueryDisposition.WRITE_APPEND    # APPEND, not truncate
            )
        )
