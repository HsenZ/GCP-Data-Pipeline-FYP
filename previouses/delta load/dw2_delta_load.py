...
def _merge_fact_table():
    ...
    WHEN MATCHED THEN UPDATE SET
      ...
      VL_n_mag            = S.VL_n_mag,
      LB_magCategory      = S.LB_magCategory,
      VL_n_depth          = S.VL_n_depth,
      LB_depthCategory    = S.LB_depthCategory,
      LB_Region           = S.LB_Region,
      LB_Country          = S.LB_Country,
      LB_place            = S.LB_place,
      DT_time             = S.DT_time,
      ...
      LB_status           = S.LB_status,
      LB_locationSource   = S.LB_locationSource,
      LB_magSource        = S.LB_magSource,
...
def run(argv=None):
    ...
    def enrich_fact(row, net_map, reg_map, type_map, mag_map, depth_map):
        ...
        insertion_date = None
        if row.get('_DT_insertion_date'):
            insertion_date = parser.parse(row['_DT_insertion_date']) if isinstance(row['_DT_insertion_date'], str) else row['_DT_insertion_date']
            insertion_date = insertion_date.replace(tzinfo=None)
